...
class WPD_Public {
    ...
    public function enqueue_scripts() {
        ...
        wp_localize_script($this->wpd, 'ajax_object', array('ajax_url' => admin_url('admin-ajax.php'),'ajax_nonce'=>wp_create_nonce('wpd_ajax_nonce')));
    ...
    function set_variable_action_filters() {
        ...
        if (isset($options['wpc-parts-position-cart']) && $options['wpc-parts-position-cart'] == 'name') {
            if ($woo_version < 2.1) {
                // Old WC versions
        ...
        if(isset($options['wpc-content-filter']))
            $append_content_filter = $options['wpc-content-filter'];
        else $append_content_filter = '0';
