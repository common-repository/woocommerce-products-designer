...
class WPD_Design {
	...
	private function add_designs_to_cart( $final_canvas_parts ) {
		...
		foreach ( $variations as $variation_name => $variation_info ) {
			...
			$product   = wpd_get_product( $variation_id );
			$variation = array();
			...
			$product = wpd_get_product( $product_id );
			...
			if ( $product->get_type() === 'variable' ) {
				...
				$newly_added_cart_item_key = $woocommerce->cart->add_to_cart( $product_id, $quantity, '', '', array( 'wpc_generated_data' => $final_canvas_parts ) );
			}
